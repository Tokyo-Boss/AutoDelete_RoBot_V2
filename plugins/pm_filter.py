...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, BOT_USERNAME
...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏪ 𝓑𝓐𝓒𝓚", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"📃 𝓟𝓐𝓖𝓔𝓢 {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
        btn.append(
            [InlineKeyboardButton("❌𝓒𝓛𝓞𝓢𝓔❌", callback_data="close_data")]
        )
    ...
    btn.insert(0, [
        InlineKeyboardButton(text="⚠️ℍ𝕆𝕎 𝕋𝕆 𝔻𝕆𝕎ℕ𝕃𝕆𝔸𝔻⚠️", url="https://t.me/tmmainchannel/4")
    ])
    try:
...
async def advantage_spoll_choker(bot, query):
    ...
    if not movies:
        return await query.answer("You Are Clicking An Old Button Which Is Expired.", show_alert=True)
    ...
    if files:
        ...
        k = await query.message.edit('This Movie IS Not Found In DataBase')
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == "private":
            ...
            return await query.answer('Im Watching YOU 👀')
    ...
    if query.data.startswith("file"):
        ...
        if f_caption is None:
            f_caption = f_caption = f"{files.file_name}"
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('Check PM, I Have Sent Files There',show_alert = True)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🏠 𝓗𝓞𝓜𝓔', callback_data='start'),
        ...
        buttons= [[
            ...
            InlineKeyboardButton('🆘 𝓗𝓔𝓛𝓟', url='https://t.me/Technomindzchat'),
            InlineKeyboardButton('𓂀 𝕐𝕆𝕌𝕋𝕌𝔹𝔼 𓂀', url='https://www.youtube.com/c/TechnoMindz')
            ],[
            InlineKeyboardButton('🧑‍💻 𝓓𝓔𝓥𝓔𝓛𝓞𝓟𝓔𝓡', url='https://t.me/TechnomindzYt'),
            InlineKeyboardButton('🤫 𝒮𝒪𝒰𝑅𝒞𝐸', url='https://t.me/Technomindzchat')
            ],[
            InlineKeyboardButton('🏠 𝓗𝓞𝓜𝓔', callback_data='start')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⏹️ 𝓑𝓤𝓣𝓣𝓞𝓝𝓢', callback_data='button')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('👩‍🦯 𝓑𝓐𝓒𝓚', callback_data='help'),
            InlineKeyboardButton('★彡[ᴀᴅᴍɪɴ]彡★', callback_data='admin')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('👩‍🦯𝓑𝓐𝓒𝓚', callback_data='help'),
            InlineKeyboardButton('♻️𝓡𝓔𝓕𝓡𝓔𝓢𝓗', callback_data='rfrsh')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    await query.answer('Im Watching YOU 👀')
      )
